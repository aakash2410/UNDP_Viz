...
import {
    ...
    Map, Activity, Users, Building2, Lightbulb, TrendingUp, Handshake, Globe, ShieldCheck, FileText, Cpu, ArrowLeft, X
} from 'lucide-react';
    ...
    const renderMetricCard = (cardData: MetricCard, icon: React.ReactNode) => (
        ...
        >
            ...
            {cardData.modalDetails && (
                <div className="mt-4 text-xs font-semibold text-blue-600 flex items-center opacity-0 group-hover:opacity-100 transition-opacity">
                    View deep dive &rarr;
                </div>
            )}
        </motion.div>
            ...
            <div className="max-w-7xl mx-auto space-y-16">
                ...
                >
                    <div className="p-8 md:p-10 border-b border-slate-100">
                        ...
                        <p className="text-lg text-slate-600 mt-3">Click on an organization to view their associated major initiatives and goals.</p>
                    </div>
                                ...
                                {data.sectionC.actors.map((actor: Actor) => (
                                    <React.Fragment key={actor.id}>
                                        <tr
                                            ...
                                            aria-expanded={expandedActorId === actor.id}
                                        >
                        ...
                        <div className="pt-6" role="tabpanel">
                            <ul className="space-y-4">
                                {activeTab === 'Opportunities' && data.sectionD.opportunities.map(item => (
                                    <li key={item.id} className="flex gap-4 bg-emerald-50/30 p-4 rounded-lg border border-emerald-100">
                                ...
                                {activeTab === 'Risks' && data.sectionD.risks.map(item => (
                                    <li key={item.id} className="flex gap-4 bg-rose-50/30 p-4 rounded-lg border border-rose-100">
                                        <div className="mt-0.5"><AlertCircle className="w-5 h-5 text-rose-500" /></div>
                                        <p className="text-slate-700">{item.text}</p>
                                    </li>
                                ))}
                                {activeTab === 'Partnerships' && data.sectionD.partnerships.map(item => (
                                    <li key={item.id} className="flex gap-4 bg-blue-50/30 p-4 rounded-lg border border-blue-100">
                                        <div className="mt-0.5"><Handshake className="w-5 h-5 text-blue-500" /></div>
                                        <p className="text-slate-700">{item.text}</p>
                                    </li>
                                ))}
                            </ul>
